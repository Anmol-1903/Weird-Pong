...
public class LocalMultiplayerController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        playerControl.LocalMultiplayer.VerticalP2.performed += VerticalP2_performed;
        playerControl.LocalMultiplayer.VerticalP2.canceled += VerticalP2_canceled;
    ...
    private void VerticalP2_performed(CallbackContext obj)
    {
        _vertical2 = obj.ReadValue<float>();
    }
    private void VerticalP2_canceled(CallbackContext obj)
    {
        _vertical2 = 0;
    }
    ...
    private void Update()
    {
        if (_p1Ready && _p2Ready)
        {
            ...
            //Player 2
            _players[1].Translate(0, _vertical2 * _speed * Time.deltaTime, 0);
    ...
    void ReadyUp()
    {
        ...
        if (!_p2Ready)
        {
            if (_vertical2 > 0)
            {
                _p2Progress += _vertical2 * Time.deltaTime / 2;
            }
            else if (_p2Progress > 0)
            {
